    ...
    public static String translateMethod(UDFType type, JavaSootMethod method, boolean isLambda) {
        ...
        if (isIgnore) {
            ...
            for (int i = 0; i < methodContext.getStmts().size(); i++) {
                ...
                if (stmt.toString().equals("interfaceinvoke r7.<org.apache.flink.streaming.api.functions.source.SourceFunction$SourceContext: void collect(java.lang.Object)>(r4)")
                    &&method.toString().equals("<com.meituan.data.rt.metrics.function.MetricsGenerateSource: void run(org.apache.flink.streaming.api.functions.source.SourceFunction$SourceContext)>")){
        ...
        for (ClassType staticClass : staticClasses) {
            staticInit.append(TAB).append(TranslatorUtils.formatType(staticClass)).append("::").append("initStaticField").append(";").append(NEW_LINE);
        }
